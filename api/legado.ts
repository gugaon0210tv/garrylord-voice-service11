...
module.exports = async (request: Request, response: Response) => {
  ...
  if (typeof voiceFormatQuery !== 'string') {
    throw new Error(
      `Invalid format ${String(voiceFormatQuery)}`,
    )
  }
  ...
  data['contentType'] =
    FORMAT_CONTENT_TYPE.get(voiceFormat)
  ...
  const ssml =
    ...
    (lexicon === ''
      ? ''
      : `<lexicon uri="${String(lexicon)}"/>`) +
    (styleName
      ? `<mstts:express-as style="${String(styleName)}" ` +
        `styledegree="${String(styleDegree ?? '')}">`
      : '') +
  ...
  data['url'] =
    String(api) + ',' + JSON.stringify(body)
